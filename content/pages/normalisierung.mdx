...
Sollen Suchergebnisse nach verschiedenen Attributwerten wie Jahr, Regionen oder Medium ausgewertet werden, so ist es ***unabdingbar***, die absoluten Zählungsergebnisse aus der Suchmaschine zu ***normalisieren***, um valide Ergebnisse zu erhalten. D.h. statt der absoluten Trefferzahlen sind unbedingt die relativen Trefferzahlen zu verwenden.

*Anmerkung zur Bedienung der aktuellen NoSke*

Metadaten wie Jahr, Region und Medium werden in der Terminologie der NoSke **text types** genannt. In der aktuellen NoSke-Version ist es möglich, bei Frequenz-Auswertungen nach *einem* solchen **text type** die korrekten, normalisierten Werte direkt anzeigen zu lassen. Dazu ist in der Anzeige eines *Frequenz*-Ergebnisses die Option *"Show relative in text type"* und/oder *"Relative density"* zu aktivieren.

Erstere liefert die Ergebnisse in hpm – *hits per million*, letztere stellt die relative Verteilung der Ergebnisse als %-Satz dar.
...
(Anmerkung: Seit 2024 bietet die NoSketch-Engine übrigens für diese häufig nachgefragte Auswertung — Trefferzahlen über die Zeit — in der Werkzeugleiste rechts oben eine eigene Funktion namens "Der Zeitsstrahl" / "Timeline" an, die auch gleich eine grafisch ansprechendere Visualisierung vornimmt).
...
| amc\_4.3    | <Download href="/assets/content/pages/normalisierung/freqs_amc_4.3_region_province_docsrc_mediatype_year_yymm_for_normalization_2024-02-09.xlsx">Download</Download>    |
